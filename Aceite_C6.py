...
#log
log = Logs.RetornaLog("--C6 Bank || Aceite de CNJs--")
...
channel.queue_declare(queue='Aceite', durable=True)
channel.queue_purge(queue='Aceite')


def inicializa_rotinas():
    #Abrindo o Banco para verificar processos existentes
    documentos_existentes = mycol.find({"Status":2, "DataAceite":"27/10/2021"})
    contador = 0
    if documentos_existentes:
        ...
        for documento in documentos_existentes:
            ...
            if resultado:
                ...
                a.DataAceite = documento['DataAceite']
                a.Status = documento['Status']
                a.Critica = documento['Critica']

                channel.basic_publish(exchange='', routing_key='Aceite', body=json.dumps(a.__dict__))
            ...
            for x in range(len(processos)):
                ...
                if resultado:
                    ...
                    if not encontrou:
                        ...
                        a.DataAceite = data_em_texto
                        a.Status = 0
                        a.Critica = ''
                        mycol._insert(para_dict(a), op_id=a.NumeroProcesso)
                        channel.basic_publish(
                            exchange='', routing_key='Aceite', body=json.dumps(a.__dict__))
                        log.info(json.dumps(a.__dict__))
...
window = tk.Tk()
window["bg"] = "white"
window.title("Interface do Robô do C6 - Aceites")
window.geometry("530x300")

#Label inicial 
label = Label(window, text='Selecione o local onde está a planilha de Aceite para gerar a fila do robô.' ,background="white" )
label.grid(row=1, column=0, ipadx='1', ipady='5', padx='1', pady='5')

#Botão com ação
B = Button(window, text="Acionar a Fila de Aceite", command=executa_acao_aceite)
